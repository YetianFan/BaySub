...
for (s in 2:nstep){
  ...
  NX = array(NA,dim=c(n,m,2))
  NY = array(NA,dim=c(n,m,2))
  for(i in 1:n){
    for(j in 1:m){
      for(l in 1:2){
        NY[i,j,l] = exp(-(Y[i,j]-alpha[l,s-1])^2/(2*ita[l,s-1]))/sqrt(2*pi*ita[l,s-1])
        NX[i,j,l] = exp(-(X[i,j]-mu[l,s-1])^2/(2*sigmaval[l,s-1]))/sqrt(2*pi*sigmaval[l,s-1])
      }
    }
  }
  ...
  for(i in 1:n){
    ...
    for(l in 1:k){
      for(j in 1:m){
        ...
        Prob[l] = Prob[l]+log(NY[i,j,Boolean])
      }
  ...
  for(i in 1:n){
    ...
    for(j in 1:m){
      ...
      PW[id,j] = PW[id,j] + log(NY[i,j,Boolean])
      
      Boolean = ((Z[i,j]!=W.try[id,j])+1)
      PW.try[id,j] = PW.try[id,j] + log(NY[i,j,Boolean])
    }
  }
  for(i in 1:k){
    for(j in 1:m){
  ...
  for(i in 1:n){
    for(j in 1:m){
      PZ[i,j,1] = log(NY[i,j,W[lambda[i,s],j]+1]) +  log(NX[i,j,1]) 
      PZ[i,j,2] = log(NY[i,j,2-W[lambda[i,s],j]]) +  log(NX[i,j,2])
      if(R[j,s]==0){
...
setwd("D:/R/paperdataresults/R code")
ptm <- proc.time()

k = 3                     # different paths from the corresponding normal tissue
nstep = 200               # number of iteration
initialtimes = 1          # initial value for variables
