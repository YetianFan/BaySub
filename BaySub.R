...
##### Author: Yetian Fan
##### Date: 2021-07-20
##### The pairs of M-value are in Data.Rdata
#####################################################################
...
alpha[,1] = runif(2,0,initialtimes)
alpha[1,1] = -alpha[1,1]
ita = matrix(NA,nrow=2,ncol=nstep)
ita[,1] = runif(2,0,initialtimes)
mu = matrix(NA,nrow=2,ncol=nstep)
mu[,1] = runif(2,0,initialtimes)
mu[1,1] = -mu[1,1]
sigmaval = matrix(NA,nrow=2,ncol=nstep)
sigmaval[,1] = runif(2,0,initialtimes)
...
Accuracy = matrix(NA,nrow=2,ncol=nstep)
for(s in 1:nstep){
  Accuracy[1,s] = ARI(lambda[,s],Target)
  Accuracy[2,s] = NMI(lambda[,s],Target)
}

return(list(Accuracy=Accuracy,lambda=lambda[,nstep],W=W))
}
...
initialtimes = 5          # initialization interval for variables

Result = BaySub(nstep,k,initialtimes)
Accuracy = Result$Accuracy
Prediction = Result$lambda
W = Result$W
